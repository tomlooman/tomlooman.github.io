---
title: "GPU Profiling & Optimization in Unreal Engine 5 'Far Far West'"
date: 2026-04-30
...
The built-in toggle can be jarring and so we should implement a blend in the foliage Material. I followed the [Smooth Fade WPO](https://raphaelgarcia.dev/lab/smooth-wpo-fade-ue5/) article and made some adjustments. The GetPrimitiveData did not function for the ISMs and I decided to use a plain old Material Parameter instead that needs to be kept in sync.
...
As the project is (going to) support DX11 on top of DX12, they will rely on DistanceField Shadows for low-spec/DX11 mode. These settings conflicted with Virtual Shadow Maps, which do not require DF Shadows for distant shadows.
