...
Substrate is now Default for materials going forward. They now call the previous system Legacy Materials. One major difference for performance considerations with Substrate is the per-pixel complexity/byte cost from material layering.

TODO: dig a little deeper in what that bytes per pixel is. We were promised performance parity.

## Nanite Foliage and Skinning
...
This release fixed some issues with mouse/cursor handling on high poll-rate mice which is scattered across the release notes.
...
Note: Don't sleep on this tool, it's incredible to see this insight into your collision and physics and you will find ways to optimize - even if it's just from realizing how much unneeded data you are loading into Chaos.
...
- They added a bunch of CVARs to control the behavior of this new partial island sleeping, currently not entirely sure what we should do with this (experimental) feature
...
- fix Data : Detect Intel 13th/14th gen Core CPUs and work around instruction sequences implicated in high crash rates w/o microcode patches. Can be up to 20% slower on affected machines for certain rare inputs, but typical decoder slow-downs are around 0.5%. No perf impact on other machines.
  - This may be significant for your game as even when not explicitly using this for compressing your packages it may affect shader compression and be causing crashes if you have a live game with UE5.
...
- FastGeo is Epic's new 5.6+ static geometry streaming solution by avoiding actor/components altogether.
...
- `r.Lumen.TraceMeshSDFs 0` - SWRT detail traces (mesh SDF tracing) is now a deprecated path (Note: since 5.6), which won't be worked on much. For scaling quality beyond SWRT global traces it is recommended to use HWRT path instead.
...
--- ??? this seems kinda huge @ detail traces?? HWRT is the way forward for Lumen which people MUST be aware of. I need to elaborate on this a lot more, and move all this way earlier into the list.

## Materials and Shaders

Added an asset registry tag to find material instances causing shader permutations.
--- ??? this one is possibly huge, I really should order this stuff on my personal highlights and add my own sauce to it. In this case I can also delete a lot of points and focus on "highlights" not ALL perf improvements.
...
--- ??? this is huge and requires people to watch the UE5 fest video on materials to make sense.
...
- ??? TSR is important for visual quality/perf so a worthwhile call out together with the others.
...
- ??? investigate this one with more insight
...
- ??? there is some talk about this priming which shows some screenshots...look this up
...
- ??? stateless is becoming very interesting for lightweight VFX, this release includes more QoL for stateless
...
- ??? not perf gain itself, but it's a valuable checkbox for perf to disable this so it's a worthwhile fix to mention.
...
- ??? async nav walking mode? what's that?
